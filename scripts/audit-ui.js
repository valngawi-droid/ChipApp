...
(async () => {
  ...
  check('Google sign-in button rendered', text().includes('Sign in with Google'));
  check('E2EE badge shown', text().includes('End-to-end encrypted'));
  ...
  check('large title + tab bar', chatsText.includes('Chats') && chatsText.includes('Settings'));
  ...
  check('conversation opened', roomText.includes('Sudah sampai rumah') || roomText.includes('Ketik pesan') || roomText.includes('Message'));
  ...
  check('encryption system notice', roomText.toLowerCase().includes('end-to-end encrypted'));
  ...
  const setText = text();
  check('grouped table sections', setText.includes('Privacy') && setText.includes('Security'));
  check('language row', setText.includes('App Language') || setText.includes('Language'));
  ...
  check('segmented filter', callsText.includes('All') && (callsText.includes('Missed') || callsText.includes('missed')));
  ...
  await new Promise((r) => setTimeout(r, 1200));
  const commText = text();
  check('community list', commText.includes('Neighbourhood Hub') || commText.includes('ChipApp Builders'));
  check('channels section', commText.includes('Tech Daily') || commText.toUpperCase().includes('CHANNELS'));
  ...
  await new Promise((r) => setTimeout(r, 1200));
  const stText = text();
  check('my status row', stText.includes('My Status') || stText.includes('Status'));
